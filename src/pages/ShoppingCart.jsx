...
const ShoppingCart = () => {
  ...
  //   const orders = JSON.parse(localStorage.getItem("orders")) || []; // Update product stock
  ...
  //     })),
  //     totalAmount: (totalPrice - discount).toFixed(2),
  //   }; // Update the main orders array in localStorage

  //   const updatedOrders = [...orders, newOrder];
  //   localStorage.setItem("orders", JSON.stringify(updatedOrders)); // Update the logged-in user's order array
  ...
  //     }
  //     return user;
  //   }); // Update the loggedInUser in localStorage to reflect the new order

  //   localStorage.setItem(
  //     "loggedInUser",
  //     JSON.stringify(updatedUsers.find((u) => u.id === loggedInUser.id))
  //   );
  ...
  const handleCheckout = () => {
    if (cart.length === 0) {
      toast.error("Your cart is empty!");
      return;
    } // Get products and logged in user
    ...
    localStorage.setItem("orders", JSON.stringify(updatedOrders)); // 🟨 Update the user's own orders inside users[]

    const updatedUsers = users.map((user) => {
      if (user.id === loggedInUser.id) {
        return {
          ...user,
          order: [...(user.order || []), ...orderItems],
        };
      }
      return user;
    });

    localStorage.setItem("users", JSON.stringify(updatedUsers)); // ✅ Clear cart
